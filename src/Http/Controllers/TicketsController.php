...
class TicketsController extends AdminController
{
    ...
    protected function form()
    {
        ...
        $editor = AdminWorkorder::config('editor', 'textarea');
        $editor_name = $editor['name'];
        if(empty($editor['config'])){
            $form->$editor_name('content','内容')->rules('required');
        }else{
            $form->$editor_name('content','内容')->rules('required')->options($editor['config']);
        }
